...
/**
 * @brief Temperature compensation algorithm (Datasheet-based)
 */
...
/**
 * @brief Pressure compensation algorithm (Datasheet-based)
 */
...
/**
 * @brief Humidity compensation algorithm (Datasheet-based)
 */
...
bool bme280_begin(int sda, int scl) {
    ...
    bme280_write_reg(0xF4, 0x27); // Mode: Normal, Temp/Pres oversampling x1
    return true;
}
...
void bme280_read(float &temp, float &hum, float &pres) {
    ...
    pres = bme280_comp_P(p_raw) / 256.0 / 100.0;
